<%@ Page Title="" Language="C#" MasterPageFile="~/UI/Pages/Teacher/MainTeacher.master" AutoEventWireup="true" CodeFile="TeacherCourse.aspx.cs" Inherits="UI_Pages_Teacher_TeacherCourse" %>
   ...
   <table id="example2" class="table table-bordered table-striped custab" style="margin-top:0%;">
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="width: 15%">درصد پایانی</th>
                                            <th style="width: 15%">جلسات برگزار شده</th>
                                            <th style="width: 10%">لیست دانشجویات</th>
                       ...
                       success: function (data) {
                           ...
                           var template = '<tr>\
                                            ...
                                            <td class="text-center">\
                                                    <a href="{{TSL}}" class="btn btn-primary btn-xs"><i class="fa fa-users"></i>دانشجویان</a>\
                                                </td>\
                                            <td class="text-center">\
                                                <a href="{{THW}}>" class="btn btn-success btn-xs"><i class="fa fa-bookmark"></i>فعالیت </a>\
                                                <a href="{{TAPC}}" class="btn btn-primary btn-xs"><i class="fa fa-check-square-o"></i>حضور و غیاب</a>\
                                            </td>\
                                        </tr>';
                           $("#_NameTerm").html(data.Term)
                           if (data.code == 200) {
                               if (data.message.length > 0) {
                                   ...
                                   for (var i in data.message) {
                                       outdata.push([
                                           data.message[i].id,
                                           data.message[i].Name,
                                           data.message[i].CodeErae,
                                           data.message[i].SumFinalPercent,
                                           data.message[i].GetSumMeeting,
                                           "<a href=\""+data.message[i].EraeSrl+'" class="btn btn-primary btn-xs"><i class="fa fa-users"></i>دانشجویان</a>',
                                           '<a href="' + data.message[i].TeacherActivity + '" class="btn btn-success btn-xs"><i class="fa fa-bookmark"></i>فعالیت </a>'+
                                           '<a href="' +data.message[i].TeacherAPChecking + '" class="btn btn-primary btn-xs"><i class="fa fa-check-square-o"></i>حضور و غیاب</a>'
                                       ]);
                                       /*
